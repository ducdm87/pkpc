...
if ( ! defined('modYOSMenuXMLVertCallbackDefined') )
{
function modYOSMenuXMLVertCallback(&$node, $args) {
...
//		$css         = 'lvl' . $level . ' item' . $node->attributes('order');
		...
		|| (isset($node->ul[0]) && ($args['end'] == 0 || $node->attributes('level') < $args['end']) && $level == 1)) {
			$css .= ' show';
			$node->ul[0]->addAttribute('class', 'vert');
		}
		...
		if (isset($args['children']) && !$args['children'])	{
			...
			foreach ($node->children() as $child) {
				if ($child->name() == 'ul') {
					...
					// dont remove children for vert
					if (!$child->attributes('class') == 'vert') {
						$node->removeChild($child);
					}
				}
			}
	...
	define('modYOSMenuXMLVertCallbackDefined', true);
}