...
class SPAControllercontent extends SPAController   {
	...
	function saveorder()
	{
		...
		for ($i = 0; $i < $total; $i ++)
		{
			...
			if ($row->ordering != $order[$i]) {
				...
				$condition = 'module = '.(int) $row->module.' AND state >= 0';
				$found = false;
...
//		$cache->clean();
		...
		$mainframe->redirect('index.php?option=com_spa_manager', $msg);
	...
	function orderContent($direction)
	{
		...
		{	
			$row =& JTable::getInstance('SPA_content','Table');
			$row->load( (int) $cid[0] );
			$row->move($direction, 'module = ' . (int) $row->module . ' AND state >= 0' );
		}

		$mainframe->redirect('index.php?option=com_spa_manager');
	}
...
//		$row->checkin();
	...
	function save()
	{
		...
		$row->cdate	=	($row->cdate != '0000-00-00 00:00:00' and $row->cdate != '')?$row->cdate:$date->toMySQL();
		...
		for ($i=0, $n=count( $orders ); $i < $n; $i++)
		{
			if ($orders[$i]->ordering >= 0)
			{
				if ($orders[$i]->ordering != $i+1)
				{
					...
					$query = 'UPDATE `#__spa_content`'
					. ' SET ordering = '. (int) $orders[$i]->ordering
					. ' WHERE id = '. $db->Quote($orders[$i]->id)
					;
					$db->setQuery( $query);
					$db->query();
				}
